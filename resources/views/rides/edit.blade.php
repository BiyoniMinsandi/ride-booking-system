...
@section('content')
    ...
    <div class="container mt-5">
        <h1 class="header-title">Edit Ride</h1> <!-- Updated header class -->
        <form action="{{ route('rides.update', $ride->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <div class="form-group">
                <label for="pickup_location">Pickup Location</label>
                <input type="text" name="pickup_location" id="pickup_location" class="form-control" value="{{ $ride->pickup_location }}" required>
            </div>
            <div class="form-group">
                <label for="dropoff_location">Dropoff Location</label>
                <input type="text" name="dropoff_location" id="dropoff_location" class="form-control" value="{{ $ride->dropoff_location }}" required>
            </div>
            <div class="form-group">
                ...
                <select name="vehicle_id" id="vehicle_id" class="form-control">
                    ...
                    @foreach($vehicles as $vehicle)
                        <option value="{{ $vehicle->id }}" {{ $ride->vehicle_id == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->type }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="status">Status</label>
                <input type="text" name="status" id="status" class="form-control" value="{{ $ride->status }}" required>
            </div>

            <div class="btn-group">
                <button type="submit" class="btn btn-purple">Update Ride</button>
